package uy.edu.fing.tse.demo.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "demo-ear.ear");

        // Current directory is the root of the "demo-web" project. Go up one level
        // and enter the "demo-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../demo-ejb/target/demo-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "demo-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/demo-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "demo-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("uy.edu.fing.tse.demo.test");
